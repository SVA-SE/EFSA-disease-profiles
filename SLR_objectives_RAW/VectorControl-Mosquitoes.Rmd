...
vector <- "mosquitoes"
dfVCmosq <- dfvc[dfvc$studyTarget_vector==vector,]
dfVCmosq$studyTarget_host <- as.factor(as.character(dfVCmosq$studyTarget_host))
...
df1 <- dfVCmosq %>% filter (studyTarget_host != "NOT HOST SPECIFIC")
...
df2 <- dfVCmosq %>% filter (studyTarget_host == "NOT HOST SPECIFIC")
...
table.left.overs <- dfVCmosq[dfVCmosq$rowID%in%left.overs$rowID,]  %>%
...
```{r references}
options(DT.options = list(scrollY="100vh",pageLength =5))
datatable(unique(references[references$Refid%in%dfVCmosq$refID,c(1,4,5,9)]), filter = 'top', rownames=FALSE)
