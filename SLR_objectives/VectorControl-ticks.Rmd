...
vector <- "ticks"
dfVCticks <- dfvc[dfvc$studyTarget_vector==vector,]
dfVCticks$studyTarget_host <- as.factor(as.character(dfVCticks$studyTarget_host))
...
<h1>External use - collar/ear tag</h1>
...
df1 <- dfVCticks %>% filter (studyTarget_host != "NOT HOST SPECIFIC")
...
efficacy.route.species.boxplots(df1 %>% filter (route == "External use - collar/ear tag") %>%drop.levels())%>% layout(font=plots.font)
...
-----------------------------------------------------------------------
###  {.bggreen data-width=200}
<h1>Subcutaneous application</h1>
...
<div class="modal-content">
<div class="modal-body">
Boxes with a dark blue contour refer to studies reporting vector MORTALITY, rather than substance efficacy (which is calculated as the mortality in the experimental group in comparison to a control group). 
    <br><br>
...
efficacy.route.species.boxplots(df1 %>% filter (route == "SUBCUTANEOUS") %>%drop.levels())%>% layout(font=plots.font)
...
-----------------------------------------------------------------------
###  {.bggreen data-width=200}
<h1>Oral administration</h1>

### 

```{r, message=FALSE, warning=FALSE}

efficacy.route.species.boxplots(df1 %>% filter (route == "ORAL: UNSPECIFIED") %>%drop.levels())%>% layout(font=plots.font)
```
...
efficacy.route.species.boxplots(df)%>% layout(font=plots.font)
...
-----------------------------------------------------------------------
###
```{r ticks.NOhost, message=FALSE, warning=FALSE}
df2 <- dfVCticks %>% filter (studyTarget_host == "NOT HOST SPECIFIC")
...
```{r}
#remember to count the row ids and table all the non used rowIDs

left.overs <- dfVCticks%>% 
...
table.left.overs <- dfVCticks[dfVCticks$rowID%in%left.overs$rowID,]  %>%
...
```{r references}
options(DT.options = list(scrollY="100vh",pageLength =5))
datatable(unique(references[references$Refid%in%dfVCticks$refID,c(1,4,5,9)]), filter = 'top', rownames=FALSE)
